...
_cfgArray = "(getText (_x >> 'author') isEqualTo 'Red Hammer Studios')" configClasses (configFile >> "CfgWeapons");

_names = [];

{
	_className = configName _x;
	_find = _className find "rhs";

	if !(_find isEqualTo -1) then
	{
		if (([_className] call BIS_fnc_itemType select 1) isEqualTo "Vest") then
		{		
